...
async def draw_rectangle(x1: int, y1: int, x2: int, y2: int) -> dict:
    ...
    try:
        ...
        # Calculate coordinates with primary width adjustment
        start_x = x1 + primary_width
        start_y = y1
        end_x = x2 + primary_width
        end_y = y2
        
        print(f"Drawing rectangle from ({start_x}, {start_y}) to ({end_x}, {end_y})")
        
        # Single press-move-release sequence
        print("Pressing mouse at start position...")
        canvas.press_mouse_input(coords=(start_x, start_y))
        time.sleep(0.5)  # Longer delay after press
        
        print("Moving mouse to end position...")
        canvas.move_mouse_input(coords=(end_x, end_y))
        time.sleep(0.5)  # Longer delay after move
        
        print("Releasing mouse at end position...")
        canvas.release_mouse_input(coords=(end_x, end_y))
        time.sleep(0.5)  # Longer delay after release
